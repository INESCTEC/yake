...
def test_n1_EN():
    ...
    result = pyake.extract_keywords(text_content)
    print(result)
    # Expected results from YAKE 0.6.0 (CORRECTED - verified against actual output)
    res = [
        ("Google", 0.02509259635302287),
        ("Kaggle", 0.027297150442917317),
        ("data", 0.07999958986489127),
    ...
    # CORRECTED: Added <kw>competitions</kw> tags since "competitions" is now in top 20
    # Removed <kw>scientists</kw> tags since "scientists" is NOT in top 20 (it's #21)
    assert (
        textHighlighted
